...
static char *test_string_length() {
  mu_assert('string_length('happy') == 5', test_string_length('happy') == 5);
    return 0;
}
static char *test_is_valid_character() {
    mu_assert('is_valid_character(' ') == 0', test_is_valid_character(' ') == 0);
    mu_assert('is_valid_character('h') == 1', test_is_valid_character('h') == 1);
    return 0;
}

static char *test_find_word_start() {
    char *str = "  happy";
    mu_assert("find_word_start('  happy') == &str[2]'", test_find_word_start(str) == &str[2]);
    return 0;
}

static char *test_find_word_terminator() {
  char *str = "happy joy", *empty="";
    mu_assert("find_word_terminator('happy joy') == &str[5]' '", test_find_word_terminator(str) == &str[5]);
    mu_assert("find_word_terminator(emptyStr) == empty", test_find_word_terminator(empty) == empty);
    return 0;
}
